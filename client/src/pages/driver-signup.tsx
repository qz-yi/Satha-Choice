...
export default function DriverAuth() {
  ...
  return (
    <div className="h-full bg-[#F3F4F6] flex flex-col font-sans overflow-y-auto" dir="rtl">
      {/* شريط علوي ثابت */}
      <div className="flex-shrink-0 px-4 pt-4 pb-2 flex items-center">
        <button
          onClick={handleBack}
          className="flex items-center justify-center gap-2 bg-white rounded-2xl px-4 py-2.5 shadow-md border border-gray-100 active:scale-95 transition-transform"
        >
          <ArrowRight className="w-5 h-5 text-orange-500" />
          <span className="font-black text-sm text-gray-700">رجوع</span>
        </button>
      </div>

      {/* المحتوى الرئيسي */}
      <div className="flex-1 flex flex-col items-center px-6 pb-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-md w-full space-y-6 mt-4">
