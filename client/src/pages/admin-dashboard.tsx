...
import { 
  ...
  MapPin, Search, Trash2, Phone, ArrowLeftRight
} from "lucide-react";
...
export default function AdminDashboard() {
  ...
  const { data: allRequests = [] } = useQuery<Request[]>({ queryKey: ["/api/requests"] });
        ...
        <header className="h-24 bg-white/80 backdrop-blur-md border-b border-gray-100 px-6 md:px-10 flex items-center justify-between z-[1000]">
          <div className="flex items-center gap-6">
            ...
            <div className="relative group">
                ...
                <input type="text" placeholder="ابحث عن كابتن..." className="bg-gray-100/50 border-2 border-transparent focus:border-orange-500 focus:bg-white outline-none rounded-2xl pr-12 pl-6 py-3 w-64 md:w-[450px] font-bold text-sm transition-all shadow-inner" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
            </div>
          </div>
        </header>
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} key="map-view" className="space-y-6">
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="space-y-4 overflow-y-auto max-h-[500px] pb-10">
                            <h3 className="font-black text-xl italic p-2 flex items-center gap-2"><Truck className="text-orange-500 w-5 h-5"/> إدارة الطلبات</h3>
                            {activeRequests.map(req => (
                                <div key={req.id} className="bg-white p-5 rounded-[30px] shadow-sm border-r-4 border-orange-500">
                                    <div className="text-[11px] text-gray-500 font-bold mb-3">الموقع: {req.location}</div>
                                    <div className="flex flex-col gap-2 mt-4">
                                        <Button onClick={() => {
                                                const onlineDrivers = allDrivers.filter(d => d.isOnline && d.status === 'approved');
                                                if (onlineDrivers.length === 0) return toast({ title: "لا يوجد سائقين متصلين", variant: "destructive" });
                                                const driverName = prompt(`اختر اسم الكابتن:\n${onlineDrivers.map(d => `- ${d.name} (ID: ${d.id})`).join('\n')}`);
                                                const selectedDriver = onlineDrivers.find(d => d.name === driverName || d.id.toString() === driverName);
                                                if(selectedDriver) assignMutation.mutate({ requestId: req.id, driverId: selectedDriver.id });
                                            }} className="w-full bg-slate-900 hover:bg-orange-500 rounded-xl h-10 text-[10px] font-black transition-colors">تحويل لسائق محدد</Button>
                                        {req.driverId && <Button variant="outline" onClick={() => cancelAssignmentMutation.mutate(req.id)} className="w-full border-red-100 text-red-500 hover:bg-red-50 rounded-xl h-10 text-[10px] font-black transition-colors">مرجوع (إلغاء السائق)</Button>}
                                    </div>
                                </div>
                            ))}
                        </div>
                        ...
                        {pendingDrivers.map((driver) => (
                            <motion.div layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} key={driver.id} className="bg-white p-6 rounded-[35px] border border-gray-100 shadow-sm relative overflow-hidden">
                                ...
                                <div className="flex gap-4 mb-6 text-right">
                                    <div className="w-14 h-14 bg-orange-50 rounded-2xl flex items-center justify-center text-xl text-orange-500 font-bold">{driver.name ? driver.name.charAt(0) : "?"}</div>
                                    <div><h4 className="font-black text-lg text-slate-800 mb-1">{driver.name}</h4><div className="flex items-center gap-2 text-xs text-gray-400 font-bold"><MapPin className="w-3 h-3" /> {driver.city}</div></div>
                                </div>
                                <div className="flex gap-3">
                                    <Button onClick={() => approveMutation.mutate(driver.id)} className="flex-1 bg-orange-500 hover:bg-black text-white rounded-[20px] font-black h-14 transition-all gap-2"><CheckCircle2 className="w-5 h-5" /> تفعيل</Button>
                                    <Button onClick={() => { if(window.confirm("حذف الطلب؟")) deleteMutation.mutate(driver.id) }} variant="ghost" className="w-14 h-14 bg-red-50 text-red-500 rounded-[20px] hover:bg-red-500 hover:text-white transition-all"><Trash2 className="w-6 h-6" /></Button>
                                </div>
                            </motion.div>
                        ))}
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {approvedDrivers.map(driver => (
                            <div key={driver.id} className="bg-white p-5 rounded-[30px] shadow-sm border border-gray-100 flex items-center justify-between transition-all">
                                ...
                                <Button onClick={() => { if(window.confirm(`هل أنت متأكد من حذف الكابتن ${driver.name}؟`)) deleteMutation.mutate(driver.id); }} variant="ghost" className="w-12 h-12 bg-red-50 text-red-500 rounded-2xl hover:bg-red-500 hover:text-white transition-all flex items-center justify-center"><Trash2 className="w-5 h-5" /></Button>
                            </div>
                        ))}
                    </div>
                </motion.div>
